...
const EMAILJS_SERVICE_ID  = 'service_250qlk8';
const EMAILJS_TEMPLATE_ID = 'template_4wpf93o';
const EMAILJS_PUBLIC_KEY  = 'uX93smIJC0vGkMTMW';
// ─────────────────────────────────────────────────────────────────────────────
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Send via EmailJS (for immediate email notification)
      await emailjs.send(
        EMAILJS_SERVICE_ID,
        EMAILJS_TEMPLATE_ID,
        {
      ...
      // 2. Optional: Send to your own backend (for logging/database)
      // Change VITE_API_URL in your client/.env file
      const API_URL = import.meta.env.VITE_API_URL;
      if (API_URL && API_URL !== 'https://your-render-url.onrender.com') {
        try {
          ...
          // We don't fail the whole form if just the logging fails
        }
      }
  ...
  return (
    <section id="contact" className="py-20 font-sans">
      ...
      <div className="flex flex-col lg:flex-row gap-12 sm:gap-20 lg:gap-32 items-start">
        ...
        >
          <div className="bg-white p-6 sm:p-10 md:p-12 rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-gray-50 relative overflow-hidden">
